...
$fetchallfromcurrent = $db->fetchallfromcurrent();
if($fetchallfromcurrent){
	echo'<table class="table table-striped table-hover" cellspacing="0" width="100%" id="curacc_table">
			<thead>
				<tr>
			...
			<tbody>';
				foreach($fetchallfromcurrent as $row){
					echo'<tr style="text-align:center;">
							...
							<td><a href="#" class="btn btn-success btn-xs"  target="_blank"><span class="glyphicon glyphicon-download-alt" aria-hidden="true"></span></a>&nbsp;
								<a data-toggle="modal" data-target="#update_curacc" class="btn btn-info btn-xs" onclick=edit_curacc("'.$row['customer_uid'].'");><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a>
		...
		<div class="modal fade" id="update_curacc" role="dialog">
			<div class="modal-dialog modal-lg" style="width:80%;">
				<div class="modal-content">
					<div class="modal-header" style="background-color:yellowgreen;color:white;">
						<button type="button" class="close" data-dismiss="modal">&times;</button>
						<h4 class="modal-title text-center"><b>Update Current Account Details</b></h4>
					</div>
					<div class="modal-body col-md-12">
						<div id="updatecuracc"></div>
					</div>
	...
	echo "<div style='text-align:center;font-family:Open Sans;font-size:20px;'><b>There Are No Current Account To Display</b></div>";
}
echo'
<script>
	$(document).ready(function() {
		var table = $("#curacc_table").DataTable({
			stateSave: true,
				"deferRender": true,
			dom: "Blfrtip",
			buttons: [
	...
	function edit_curacc(customer_uid){
		$.ajax({
            type: "POST",
            url:"../get_files/Customers/Current/edit_curacc_dtls.php",
            data: {
				customer_uid : customer_uid,
            },
            success:function(result){
				$("#updatecuracc").html(result);
            }
        });	
	}
</script>'; 
