...
$fetchallfromrd = $db->fetchallfromrd();
if($fetchallfromrd){
	echo'<table class="table table-striped table-hover" cellspacing="0" width="100%" id="rdacc_table">
			<thead>
				<tr>
				...
				foreach($fetchallfromrd as $row){
					echo'<tr style="text-align:center;">
							...
							<td><a href="#" class="btn btn-success btn-xs"  target="_blank"><span class="glyphicon glyphicon-download-alt" aria-hidden="true"></span></a>&nbsp;
								<a data-toggle="modal" data-target="#update_rdacc" class="btn btn-info btn-xs" onclick=edit_rdacc("'.$row['customer_uid'].'");><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a>
		...
		<div class="modal fade" id="update_rdacc" role="dialog">
	...
	echo "<div style='text-align:center;font-family:Open Sans;font-size:20px;'><b>There Are No RD Account To Display</b></div>";
}  
echo'
<script>
	$(document).ready(function() {	
		var table = $("#rdacc_table").DataTable({
			stateSave: true,
			"deferRender": true,
			dom: "Blfrtip",
			buttons: [
	...
	function edit_rdacc(customer_uid){
		$.ajax({
            type: "POST",
            url:"../get_files/Customers/RD/edit_rdacc_dtls.php",
            data:{
				customer_uid : customer_uid,
            },
            success:function(result){
				$("#updaterdacc").html(result);
            }
        });	
	}	
</script>';
