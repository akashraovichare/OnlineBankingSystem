...
$fetchallfromfd = $db->fetchallfromfd();
if($fetchallfromfd){
	echo'<table class="table table-striped table-hover" cellspacing="0" width="100%" id="fdacc_table">
			<thead>
				<tr>
				...
				foreach($fetchallfromfd as $row){
					echo'<tr style="text-align:center;">
							...
							<td><a href="#" class="btn btn-success btn-xs"  target="_blank"><span class="glyphicon glyphicon-download-alt" aria-hidden="true"></span></a>&nbsp;
								<a data-toggle="modal" data-target="#update_fdacc" class="btn btn-info btn-xs" onclick=edit_fdacc("'.$row['customer_uid'].'");><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a>
		...
		<div class="modal fade" id="update_fdacc" role="dialog">
	...
	echo "<div style='text-align:center;font-family:Open Sans;font-size:20px;'><b>There Are No FD Account To Display</b></div>";
}		  
echo'
<script>
	$(document).ready(function() {	
		var table = $("#fdacc_table").DataTable({
			stateSave: true,
			"deferRender": true,
			dom: "Blfrtip",
			buttons: [
	...
	function edit_fdacc(customer_uid){
		$.ajax({
            type: "POST",
            url:"../get_files/Customers/FD/edit_fdacc_dtls.php",
            data: {
				customer_uid : customer_uid,
            },
            success:function(result){
				$("#updatefdacc").html(result);
            }
        });	
	}	
</script>';
