...
$fetchallfromtehsil = $db->fetchallfromtehsil(/*$orgid*/);
if($fetchallfromtehsil){
	echo'<table class="table table-striped table-compact table-hover" cellspacing="0" width="100%" id="tehsil_table">
			<thead>
				<tr>
					<th style="text-align:center;"><h4>Tehsil UID</h4></th>
					<th style="text-align:center;"><h4>Tehsil Name</h4></th>
			...
			<tbody>';
				foreach($fetchallfromtehsil as $row){
					echo'<tr style="text-align:center;">
							<td>'.$row["tehsil_uid"].'</td>
							<td>'.$row["tehsil_name"].'</td>
							<td>'.$row["district"].'</td>
							<td>'.$row["state"].'</td>
							<td>
								<a href="#" class="btn btn-success btn-xs"  target="_blank"><span class="glyphicon glyphicon-download-alt" aria-hidden="true"></span></a>&nbsp;
								<a data-toggle="modal" data-target="#update_tehsil" onclick=edit_tehsil("'.$row["tehsil_uid"].'"); class="btn btn-info btn-xs"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a>
		...
		<div class="modal fade" id="update_tehsil" role="dialog">
...
<script>
	$(document).ready(function() {
		var table = $("#tehsil_table").DataTable({
			stateSave: true,
			"deferRender": true,
			dom: "Blfrtip",
			buttons: [
	...
	function edit_tehsil(tehsil_uid){
		$.ajax({
            type: "POST",
            url:"../get_files/Place/Tehsil/updt_tehsil_dtls.php",
            data:{
				tehsil_uid : tehsil_uid,
            },
            success:function(result){
				$("#updatetehsil").html(result);
            }
        });	
	}
</script>';	  
